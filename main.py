...
# LISTA E QYTETEVE (Sigurohu që emrat këtu janë identikë me butonat)
CHANNELS = {
...
def collect_report(message):
    ...
    # Injoro klikimet e butonave që të mos trajtohen si lajm i ri
...
def handle_channel_selection(message):
    ...
    if user_id in user_data:
        ...
        # Butonat e konfirmimit (Duhet të përputhen fiks me Handle më poshtë)
...
def handle_final_confirmation(message):
    ...
    if not data or 'target_channel' not in data:
        bot.send_message(user_id, "Ein Fehler ist aufgetreten. Bitte mit /start neu starten.", reply_markup=types.ReplyKeyboardRemove())
        return
    ...
    if message.text == "Ja, posten ✅":
        try:
            ...
            bot.send_message(user_id, "❌ Fehler: Bot ist kein Admin in diesem Kanal.")
    else:
        bot.send_message(user_id, "❌ Abgebrochen.", reply_markup=types.ReplyKeyboardRemove())
